...
-(void) subRender: (NSDictionary*)dictionary {
    ...
    // here , set defalut
    if (dictionary[k_JR_CONT_Insets])
        [self setContentEdgeInsets: [FrameHelper convertCanvasEdgeInsets: [RectHelper parseUIEdgeInsets:  dictionary[k_JR_CONT_Insets]]]];
    if (dictionary[k_JR_FGIMG_Insets])
        [self setImageEdgeInsets: [FrameHelper convertCanvasEdgeInsets:[RectHelper parseUIEdgeInsets: dictionary[k_JR_FGIMG_Insets]]]];
    if (dictionary[k_JR_TITLE_Insets])
        [self setTitleEdgeInsets: [FrameHelper convertCanvasEdgeInsets:[RectHelper parseUIEdgeInsets: dictionary[k_JR_TITLE_Insets]]]];
    
    // title font
    [JsonViewHelper setLabelFontAttribute: self.titleLabel config:dictionary];
    if (dictionary[JSON_FONT_N_COLOR])
        [self setTitleColor: [ColorHelper parseColor: dictionary[JSON_FONT_N_COLOR]] forState:UIControlStateNormal];
    if (dictionary[JSON_FONT_H_COLOR])
        [self setTitleColor: [ColorHelper parseColor: dictionary[JSON_FONT_H_COLOR]] forState:UIControlStateHighlighted];
    if (dictionary[JSON_FONT_S_COLOR])
        [self setTitleColor: [ColorHelper parseColor: dictionary[JSON_FONT_S_COLOR]] forState:UIControlStateSelected];
    int size = [dictionary objectForKey: JSON_FONT_SIZE] ? [[dictionary objectForKey: JSON_FONT_SIZE] integerValue] : 20 ;
    self.titleLabel.font =
    [self.titleLabel.font fontWithSize: [FrameTranslater convertFontSize: size]];
    ...
    if (bgImage) {
        ...
        if (!hightlightBGImage)
            hightlightBGImage = [ImageHelper applyingAlphaToImage: bgImage alpha:0.5];
        UIImage* selectedBGImage = [UIImage imageNamed:[dictionary objectForKey: k_JR_BGS_Image]];
        if (!selectedBGImage)
            selectedBGImage = [ImageHelper applyingAlphaToImage: bgImage alpha:0.5];
    ...
    if (fgImage) {
        ...
        if (!hightlightFGImage)
            hightlightFGImage = [ImageHelper applyingAlphaToImage: fgImage alpha:0.5];
        UIImage* selectedFGImage = [UIImage imageNamed:[dictionary objectForKey: k_JR_FGS_Image]];
        if (!selectedFGImage)
            selectedFGImage = [ImageHelper applyingAlphaToImage: fgImage alpha:0.5];
